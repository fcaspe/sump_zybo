...
##Clock signal (125 MHz generated by the ethernet controller from a 25MHz crystal).
set_property -dict {PACKAGE_PIN K17 IOSTANDARD LVCMOS33} [get_ports sysclk]
create_clock -period 8.000 -name sys_clk_pin -waveform {0.000 4.000} -add [get_ports sysclk]

# Clocks generated with clock wizard do not appear in any xdc file (not even in the clock_wizard generated ones)
# they can be viewed with 'report_clocks' TCL command. https://forums.xilinx.com/t5/Timing-Analysis/create-generated-clock-thru-MMCM/td-p/784744
# Moreover,the XDC files of the IP's shouldnt be modified by the user.
#create_clock -period 8.000 -name sys_clk_pin -waveform {0.000 4.000} -add [get_ports sysclk]
...
##Buttons
set_property -dict {PACKAGE_PIN K18 IOSTANDARD LVCMOS33} [get_ports resetSwitch]
set_property -dict {PACKAGE_PIN P16 IOSTANDARD LVCMOS33} [get_ports triggerSwitch]
...
set_property -dict {PACKAGE_PIN W15 IOSTANDARD LVCMOS33} [get_ports jc_1_tx]
set_property -dict { PACKAGE_PIN T11   IOSTANDARD LVCMOS33     } [get_ports { jc_2_rx }]; #IO_L1P_T0_34 Sch=jc_p[2]
...
set_property -dict {PACKAGE_PIN V18 IOSTANDARD LVCMOS33} [get_ports exClock]
## Define clock constraint here for the external clock. (NOT USED AS I can't manage to close timing)
#create_clock -period 10.000 -name ext_clk_pin -waveform {0.000 5.000} -add [get_ports exClock]

# There's no optimal route I can find for exClock, in any pin of the Zybo-Z710
# When not using extClock, there is no invalid timing in the design.
# Maybe the timing closure problem comes only from exClock?
# Best thing for now is not to constraint exClock, just allow it to be routed using fabric (No MRCC available from the Zybo Z7)
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets exClock]
#set_clock_groups -asynchronous -group ext_clk_pin -group sys_clk_pin
